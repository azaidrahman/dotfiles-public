...
function main(folder, seconds)
    ...
    local filename = mp.get_property("media-title") .. " " .. "[" ..
                         math.floor(start_time) .. "-" .. math.floor(end_time) ..
                         "]"
    local clean_filename = string.gsub(filename, "[/\\:%*%?\"|]", "_")
    mp.commandv("dump-cache", start_time, end_time,
                folder .. clean_filename .. ".ts")
    mp.commandv("dump-cache", start_time, end_time,
                folder .. clean_filename .. ".mkv")
    ov.data = "{\\an4}{\\b1}{\\fs18}{\\1c&H00FFFF&}" .. "Saving Clip" ..
                  "\n{\\an4}{\\b1}{\\fs18}{\\1c&H00FFFF&}" .. folder ..
                  clean_filename
    ov:update()
